            ...
            <article <?php post_class(); ?>>
                    ...
                    <?php if ( wp_attachment_is_image() ) : ?>
                        <div class="entry-meta entry-content" itemscope itemtype="http://schema.org/ImageObject">
                            <?php
                                ...
                                printf( __( 'Tamanho total da imagem: %s pixels', 'odin' ), sprintf( '<a href="%1$s" title="%2$s" itemprop="image"><span itemprop="width">%3$s</span> &times; <span itemprop="height">%4$s</span></a>', wp_get_attachment_url(), esc_attr( __( 'Link da imagem completa', 'odin' ) ), $metadata['width'], $metadata['height'] ) );
                            ?>
                        </div><!-- .entry-meta -->
                    <?php endif; ?>
                </header><!-- .entry-header -->
                <div class="entry-content">
                    <?php if ( ! empty( $post->post_parent ) ) : ?>
                        <p class="page-title" itemprop="associatedMedia"><a href="<?php echo get_permalink( $post->post_parent ); ?>" title="<?php echo esc_attr( sprintf( __( 'Voltar para %s', 'odin' ), strip_tags( get_the_title( $post->post_parent ) ) ) ); ?>" rel="gallery"><?php printf( __( '<span class="meta-nav">&larr;</span> %s', 'odin' ), get_the_title( $post->post_parent ) ); ?></a></p>
                    <?php endif; ?>
